...
int bank(int atm_out_fd[], Command *cmd, int *atms_remaining) {
  ...
  if( check_valid_atm(i) != SUCCESS) return ERR_UNKNOWN_ATM;
	...
	MSG_OK(&bankcmd,0,f,t,a);
      ...
      else MSG_ACCUNKN(&bankcmd,i,t);
      result = checked_write(atm_out_fd[i], &bankcmd, MESSAGE_SIZE);
    }

    else if (c == WITHDRAW){
	if (check_valid_account(f) == SUCCESS){
        	if (a > accounts[f]) MSG_NOFUNDS(&bankcmd, i, f,a);
		else {
			accounts[t] -= a;
			MSG_OK(&bankcmd,0,f,t,a);
		}
        }
        else MSG_ACCUNKN(&bankcmd,i,f);
	result = checked_write(atm_out_fd[i], &bankcmd, MESSAGE_SIZE);
    }

    else if(c == TRANSFER){
	if((check_valid_account(f) == SUCCESS) && (check_valid_account(t) == SUCCESS)){
		if (a > accounts[f]) MSG_NOFUNDS(&bankcmd, i, f, a);
        	else{
			accounts[f]-=a;
			accounts[t]+=a;
